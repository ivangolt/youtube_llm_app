...
st.sidebar.info(
    "This App has been developed using Whisper and Langchain (LLM). For the Q&A it uses gpt-3.5-turbo (4,097 tokens)"
)
    ...
    if st.button("Extract Audio and Transcribe"):
        if url:
            try:
                ...
                yt.streams.filter(
                    progressive=True, file_extension=video_extension
                ...
                try:
                    ...
                    if os.path.exists(audio_filepath):
                        ...
                        vStore = Chroma.from_texts(
                            [result["text"]],
                            embeddings,
                            metadatas=[{"source": "Video Transcription"}],
                        )
                        ...
                        st.error(
                            "Audio file not found. Please check the audio extraction process."
                        )
